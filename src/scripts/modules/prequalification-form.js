export function initPrequalificationForm(endpoint, submitForm) {
  const form = document.querySelector("#prequalification-form");

  if (!form) return;

  const questions = Array.from(form.querySelectorAll(".prequal-question"));
  ...
  const header = document.querySelector("#prequal-header");

  const progressContainer = document.querySelector("#prequal-progress-bar");
  ...
  const updateButtons = () => {
    ...
    nextButton.classList.toggle("opacity-40", !answered);

    nextButton.classList.toggle("cursor-not-allowed", !answered);
  ...
  form.addEventListener("submit", async (event) => {
    ...
    const submitButton = form.querySelector("#prequal-submit");
    ...
    let resultField = form.querySelector('input[name="qualification"]');
    ...
    resultField.value = qualified ? "qualified" : "not-qualified";
    ...
    Object.entries(answerLabels).forEach(([key, question]) => {
      ...
      if (selectedValue !== null) {
        formData[question] = answerValues[selectedValue] || selectedValue;
      }
    });
    ...
    formData["Qualification"] = qualified ? "Qualified" : "Not Qualified";

    delete formData["qualification"];

    try {
      await submitForm(form, endpoint, formData);
